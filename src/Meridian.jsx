  ...
  aree:[
    ...
    {id:"lavoro",icon:"🔥",   nome:"Ingegno",   color:"#ff7b54", cat:[
      ...
      {id:"carr",icon:"🚀",   nome:"Carriera",    h:[
        ...
        {id:"h27",nome:"Opportunità commerciali",icon:"🚀",s:"green",note:"",freq:{type:"none"},logType:"nessuno",milestones:[]},
        {id:"h25",nome:"Gestione delle persone",icon:"👥",s:"green",note:"",freq:{type:"none"},logType:"nessuno",milestones:[]},
    ...
    {id:"relazioni",icon:"🌱",nome:"Legami",    color:"#c084fc", cat:[
      ...
      {id:"amici",icon:"✨",  nome:"Amici",       h:[
        ...
        {id:"h43",nome:"Scuola",icon:"🏫",s:"green",note:"",freq:{type:"none"},logType:"nessuno",milestones:[]},
      ...
      {spese.length>0&&(
        <div style={{display:"flex",justifyContent:"space-between",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}>
            ...
            <span style={{fontSize:14,fontWeight:700,color:"#f87171",opacity:0.7,
              fontFamily:"'JetBrains Mono',monospace"}}>-{fmt(totAnnuo)}</span>
          </div>
        </div>
      )}
    ...
    <div>
          ...
          return(
            <button key={a.id} onClick={()=>setFilterArea(sel?null:a.id)}
              style={{flex:1,padding:"6px 4px",borderRadius:10,
                ...
                border:"1.5px solid "+(sel?a.color:"rgba(255,255,255,0.08)"),
                display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
              <span style={{fontSize:14,lineHeight:1}}>{a.icon||"●"}</span>
              <span style={{fontSize:9,fontWeight:600,
                color:sel?a.color:Z.t3}}>{a.nome}</span>
            </button>
          );
        })}
      </div>
      ...
      {byCat.length===0&&(
        <div style={{background:Z.s1,border:`1px solid ${Z.b}`,borderRadius:14,
          textAlign:"center",padding:"24px 20px",color:Z.t2,fontSize:14}}>
        ...
        <div key={grp.catId} style={{background:Z.s1,
          ...
          <div onClick={()=>setOpenCats(o=>({...o,[grp.catId]:!isCatOpen(grp.catId)}))}
            style={{padding:"7px 12px 6px",display:"flex",
              ...
              borderBottom:isCatOpen(grp.catId)?`1px solid ${Z.b}`:"none",
              cursor:"pointer"}}>
          ...
          {isCatOpen(grp.catId)&&grp.habits.map((h,i)=>(
...
function TaskScreen({data,setData,onAdd,showToast,onEditTask}){
  ...
  // Gruppi per scadenza
  const remainingT=pend.filter(t=>!urg.includes(t));
  const todayStr=tod();
  const endWeek=new Date();endWeek.setDate(endWeek.getDate()+7);
  const endWeekStr=endWeek.toISOString().split("T")[0];
  const taskGroups=remainingT.length?[
    {key:"oggi",   label:"Oggi",            items:remainingT.filter(t=>t.deadline===todayStr)},
    {key:"sett",   label:"Questa settimana", items:remainingT.filter(t=>t.deadline&&t.deadline>todayStr&&t.deadline<=endWeekStr)},
    {key:"dopo",   label:"Prossimamente",    items:remainingT.filter(t=>t.deadline&&t.deadline>endWeekStr)},
    {key:"nodata", label:"Senza data",        items:remainingT.filter(t=>!t.deadline)},
    ...
    <div>
      ...
      {urg.length>0&&(
        <div style={{background:"rgba(248,113,113,0.06)",
          ...
          <div style={{padding:"10px 14px 6px",fontSize:11,color:"#f87171",
            fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",
            fontFamily:"'JetBrains Mono',monospace"}}>
            Scadute · {urg.length}
          </div>
          {urg.map((t,i)=>(
            <div key={t.id} style={{borderTop:i>0?"1px solid rgba(248,113,113,0.15)":"none"}}>
    ...
    <div style={{marginBottom:2}}>
      <div onClick={()=>setOpenG(isOpen?null:g.id)}
        style={{display:"flex",alignItems:"center",gap:10,
          ...
          border:`1px solid rgba(255,255,255,${isOpen?"0.07":"0.05"})`,
          cursor:"pointer"}}>
        <div style={{width:32,height:32,flexShrink:0,position:"relative",
          ...
          <svg width={32} height={32} style={{position:"absolute",top:0,left:0}}>
            ...
            <circle cx={16} cy={16} r={13} fill="none"
              ...
              strokeDasharray={`${(2*Math.PI*13*(done100?1:(tp||0)/100)).toFixed(1)} ${(2*Math.PI*13).toFixed(1)}`}
              strokeLinecap="round"
              transform="rotate(-90 16 16)"/>
          </svg>
          <span style={{fontSize:8,fontWeight:800,color:done100?"#00e5a0":pc,
            fontFamily:"'JetBrains Mono',monospace",position:"relative",zIndex:1}}>
            {done100?"✓":(tp!==null?tp:"")}
          </span>
        </div>
        <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center"}}>
          <div style={{fontSize:14,fontWeight:done100?400:600,
            color:done100?"#00e5a0":Z.t,
            overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
            textDecoration:done100?"line-through":"none",marginBottom:gt.length?4:0}}>
            {g.text}
          </div>
          {gt.length>0&&(
            <div style={{height:2,background:Z.s3,borderRadius:1,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${tp||0}%`,
                background:pc,borderRadius:1,transition:"width 0.4s"}}/>
            </div>
          )}
          {dl&&(
            <div style={{fontSize:9,fontWeight:600,marginTop:3,
              color:dl.u?"#f87171":"rgba(250,204,21,0.7)",
              fontFamily:"'JetBrains Mono',monospace"}}>
              ⏰ {dl.txt}
            </div>
          )}
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
          <button onClick={e=>{e.stopPropagation();setEditG(g);}}
            style={{width:28,height:28,borderRadius:8,
              ...
              border:`1px solid ${Z.b}`,color:Z.t3,fontSize:12,
              display:"flex",alignItems:"center",justifyContent:"center"}}>
            <span style={{fontSize:11,fontWeight:700,opacity:0.6}}>✎</span>
          </button>
          <span style={{fontSize:13,color:Z.t3,opacity:0.5,display:"inline-block",
        ...
        <div style={{background:"rgba(255,255,255,0.015)",
          ...
          {gt.length===0&&addTo!==g.id&&(
            <div style={{padding:"10px 13px"}}>
              <span style={{fontSize:12,color:Z.t3,fontStyle:"italic"}}>
                Nessun traguardo
              </span>
            </div>
          )}
          ...
          {gt.map((t,i)=>{
            ...
            return(
              <div key={t.id} style={{padding:"7px 11px",
                borderTop:i===0?"none":`1px solid ${Z.b}`}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <div style={{width:18,height:18,borderRadius:"50%",flexShrink:0,
                    background:tpct>=100?"transparent":tc,
                    border:`1.5px solid ${tc}`,
                    display:"flex",alignItems:"center",justifyContent:"center",
                    cursor:"pointer",transition:"all 0.15s"}}
                    onClick={e=>{e.stopPropagation();
                      updT(g.id,{...t,pct:tpct>=100?0:100,done:tpct<100});}}>
                    {tpct>=100&&<span style={{fontSize:8,color:tc,fontWeight:900}}>✓</span>}
                  </div>
                  {editingTId===t.id?(
                    <input value={editTxt}
                    ...
                    <React.Fragment>
                      <span style={{fontSize:13,flex:1,color:tpct>=100?Z.t3:Z.t,
                        textDecoration:tpct>=100?"line-through":"none",
                        overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                        {t.text}
                      </span>
                      <button
                        onClick={e=>{e.stopPropagation();setEditingTId(t.id);setEditTxt(t.text);}}
                        style={{background:"transparent",border:"none",
                          color:Z.t3,fontSize:12,padding:"0 4px",
                          cursor:"pointer",flexShrink:0,opacity:0.5}}>{"✎"}
                      </button>
                    </React.Fragment>
                  )}
                  ...
                  <button onClick={e=>{e.stopPropagation();delT(g.id,t.id);}}
                    style={{color:Z.t3,fontSize:15,opacity:0.2,
                      padding:"0 2px",flexShrink:0,lineHeight:1}}>×</button>
                </div>
                <div onClick={e=>e.stopPropagation()} style={{paddingLeft:26}}>
                  <input type="range" min={0} max={100} step={5} value={tpct}
                    onChange={e=>{const v=parseInt(e.target.value);updT(g.id,{...t,pct:v,done:v>=100});}}
                    style={{width:"100%",height:3,appearance:"none",WebkitAppearance:"none",
                      background:`linear-gradient(90deg,${tc} ${tpct}%,${Z.b2} ${tpct}%)`,
                      borderRadius:2,cursor:"pointer",outline:"none",accentColor:tc}}/>
                </div>
              </div>
            );
          })}
          ...
          {addTo===g.id?(
            <div style={{display:"flex",gap:6,padding:"8px 12px",
              borderTop:`1px solid ${Z.b}`}} onClick={e=>e.stopPropagation()}>
              <input value={newT} onChange={e=>setNewT(e.target.value)}
                ...
                placeholder="Nuovo traguardo..." autoFocus
                style={{flex:1,background:Z.s1,border:`1px solid ${Z.b2}`,
                  borderRadius:7,padding:"7px 10px",color:Z.t,fontSize:13,outline:"none"}}/>
              <button onClick={()=>addT(g.id)}
                style={{padding:"7px 12px",background:pc,borderRadius:7,
                  color:"#0a0a0a",fontSize:14,fontWeight:800}}>+</button>
              <button onClick={()=>{setAddTo(null);setNewT("");}}
                style={{padding:"7px 8px",background:Z.s1,borderRadius:7,color:Z.t3,fontSize:12}}>✕</button>
            </div>
          ):(
            <button onClick={e=>{e.stopPropagation();setAddTo(g.id);setNewT("");}}
              style={{display:"flex",alignItems:"center",gap:5,
                ...
                borderTop:`1px solid ${Z.b}`,fontSize:11,color:Z.t3}}>
              <span style={{fontSize:13}}>+</span> Traguardo
            </button>
          )}
        </div>
      )}
...
function Obiettivi({data,setData,showToast,editG,setEditG,newG,setNewG}){
  ...
  const updT=(gId,updatedTask)=>setData(d=>({...d,goals:d.goals.map(g=>{
    if(g.id!==gId)return g;
    const tasks=(g.tasks||[]).map(t=>t.id===updatedTask.id?updatedTask:t);
...
function Lucius({data,tr}){
  ...
  const {habits,todayH,doneN,pct,ws,topStreak,pending,goals,avgGoal,context}=useMemo(()=>{
    const habits=flatH(data.aree);
    const ts=tod();
    const todayH=habits.filter(h=>isExpected(h,ts));
    const doneN=todayH.filter(h=>(tr[h.id]||{})[ts]).length;
    const pct=todayH.length?Math.round(doneN/todayH.length*100):0;
    ...
    - Obiettivi: ${goals.length} in corso, ${avgGoal}% avanzamento medio
    - Dettaglio obiettivi: ${goals.slice(0,3).map(g=>`${g.text} (${g.pct||0}%)`).join(", ")||"nessuno"}
    `;
    return {habits,todayH,doneN,pct,ws,topStreak,pending,goals,avgGoal,context};
  },[data,tr]);

  const sendMsg=async(userMsg)=>{
    const msg=userMsg||"Dammi una riflessione sui miei dati di questa settimana: cosa sta andando bene, cosa migliorare, e un'azione concreta per i prossimi 3 giorni.";
    ...
    <div>
      ...
      <div style={{marginBottom:16}}>
        <div style={{fontSize:10,fontWeight:700,color:Z.t3,letterSpacing:"0.1em",
          ...
          fontFamily:"'JetBrains Mono',monospace"}}>Storico 7 giorni</div>

        {/* Barre giornaliere */}
        {(()=>{
          const habits=flatH(data.aree).filter(h=>h.freq&&h.freq.type==="daily");
          const days=d7();
          const bars=days.map(day=>{
            const done=habits.filter(h=>(tr[h.id]||{})[day]).length;
            const pct=habits.length?Math.round(done/habits.length*100):0;
            return{day,done,tot:habits.length,pct};
          });
            ...
            <div style={{background:Z.s1,borderRadius:14,padding:"14px 12px",
              ...
              <div style={{display:"flex",alignItems:"flex-end",gap:5,height:70}}>
                {bars.map((b,i)=>{
                  ...
                  const h=Math.max(4,Math.round((b.pct/100)*60));
                  const col=b.pct>=80?"#2ea34e":b.pct>=50?"#b09018":b.pct>=20?"#c46828":"#444";
                  return(
                    <div key={b.day} style={{flex:1,display:"flex",flexDirection:"column",
                      alignItems:"center",gap:4}}>
                      <div style={{fontSize:9,color:b.pct>0?col:Z.t3,fontWeight:700}}>
                        {b.pct>0?b.pct+"%":""}
                      </div>
                      <div style={{width:"100%",height:60,display:"flex",
                        alignItems:"flex-end",justifyContent:"center"}}>
                        <div style={{width:"80%",height:h,
                          background:isToday?col:col+"88",
                          borderRadius:"3px 3px 0 0",
                          transition:"height 0.3s",
                          boxShadow:isToday?`0 0 8px ${col}60`:"none"}}/>
                      </div>
                      <div style={{fontSize:9,color:isToday?Z.t:Z.t3,
        ...
        {/* Dot grid per singola abitudine */}
        {(()=>{
          const days=d7();
          const today=tod();
          const dailyH=flatH(data.aree).filter(h=>h.freq&&h.freq.type==="daily");
          return(
            <div style={{background:Z.s1,borderRadius:14,padding:"12px 14px",
              ...
              <div style={{display:"flex",alignItems:"center",gap:3,marginBottom:8}}>
                ...
                {days.map(day=>{
                  ...
                  const label=new Date(day+"T12:00:00").toLocaleDateString("it-IT",{weekday:"short"}).slice(0,3);
                  return(
                    <div key={day} style={{flex:1,textAlign:"center",fontSize:8,
                ...
                return(
                  <div key={h.id} style={{display:"flex",alignItems:"center",
                    ...
                    <div style={{width:90,flexShrink:0,display:"flex",alignItems:"center",gap:5}}>
                      ...
                      <span style={{fontSize:10,color:Z.t2,
                        overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                        {h.nome}
                      </span>
                    </div>
                    {days.map(day=>{
                      ...
                      const isToday=day===today;
                      return(
                        <div key={day} style={{flex:1,aspectRatio:"1",
                          borderRadius:3,
                          background:done?aColor:"rgba(255,255,255,0.06)",
                          boxShadow:done&&isToday?`0 0 5px ${aColor}80`:"none",
                          border:isToday?`1px solid ${aColor}30`:"none"}}/>
                      );
                    })}
                  </div>
                );
      ...
      {history.length===0?(
        <div className="up d1" style={{textAlign:"center",padding:"28px 20px",
          ...
          <h2 style={{fontSize:16,fontWeight:700,color:Z.t,marginBottom:6}}>Parla con Lucius</h2>
          <p style={{fontSize:13,color:Z.t2,lineHeight:1.6,marginBottom:14}}>
            Analizza i tuoi dati e ricevi una riflessione personalizzata.
          </p>
          <button onClick={()=>sendMsg()} disabled={loading}
            style={{padding:"12px 28px",background:SILVER,border:"none",borderRadius:12,
              color:"#0a0a0a",fontSize:14,fontWeight:700,
              opacity:loading?0.5:1}}>
            {loading?"In corso...":"Inizia"}
          </button>
        </div>
      ):(
        <div style={{background:Z.s1,border:`1px solid ${Z.b}`,borderRadius:16,overflow:"hidden"}}>
          {/* Messaggi */}
          <div ref={histRef} style={{maxHeight:320,overflowY:"auto",padding:"14px 14px 8px"}}>
            {history.map((m,i)=>(
              <div key={i} style={{marginBottom:12,
                ...
                <div style={{maxWidth:"85%",padding:"10px 12px",borderRadius:12,
            ...
            {loading&&(
              <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:12}}>
                ...
                <div style={{padding:"10px 14px",background:Z.s2,borderRadius:12,
          ...
          {/* Input */}
          <div style={{borderTop:`1px solid ${Z.b}`,padding:"10px 12px",
            ...
            <button onClick={()=>input.trim()&&sendMsg(input)} disabled={loading||!input.trim()}
              style={{padding:"9px 14px",background:input.trim()?SILVER:"rgba(200,200,212,0.15)",
                borderRadius:10,color:"#0a0a0a",fontSize:15,fontWeight:700,
      ...
      {history.length>0&&!loading&&(
        <button onClick={()=>setHistory([])}
          style={{marginTop:8,fontSize:11,color:Z.t3,padding:"6px 12px",
            background:"transparent",border:`1px solid ${Z.b}`,borderRadius:20}}>
          Nuova conversazione
        </button>
      )}
    </div>
      ...
      <div
        ...
        <div style={{flex:1,minWidth:0,cursor:"pointer",display:"flex",
          ...
          <span style={{flex:1,fontSize:14,color:t.done?Z.t3:Z.t,
            ...
            textDecoration:t.done?"line-through":"none",lineHeight:1.35,wordBreak:"break-word"}}>
            {t.text}
          </span>
          {urgent&&(
          ...
          {!urgent&&d2&&(
            <span style={{fontSize:10,color:Z.t3,flexShrink:0}}>
              {d2.txt}
            </span>
          )}
          {aName&&(
            <span style={{fontSize:9,color:aCol,fontWeight:600,
              padding:"2px 6px",border:`1px solid ${aCol}50`,
...
const HOME_ORDER=["h1","h2","h14","h6","h5","h10","h3","h51"];
  ...
  return(
    <div style={{paddingBottom:80}}>
      ...
      {/* ABITUDINI */}
      <div className="up d1" style={{padding:"20px 16px 0",marginBottom:40}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:12,flex:1,minWidth:0}}>
            <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.3)",letterSpacing:"0.12em",textTransform:"uppercase"}}>Abitudini</span>
            {todayH.length>0&&(
              <div style={{display:"flex",alignItems:"center",gap:7,flex:1,minWidth:0}}>
                <div style={{flex:1,height:2,background:"rgba(255,255,255,0.06)",borderRadius:1,overflow:"hidden",maxWidth:80}}>
                  <div style={{height:"100%",width:`${pct}%`,background:allDone?"#00e5a0":SILVER,borderRadius:1,transition:"width 0.6s ease"}}/>
                </div>
                <span style={{fontSize:11,color:allDone?"#00e5a0":"rgba(255,255,255,0.3)",fontWeight:600}}>{doneToday.length}/{todayH.length}</span>
              </div>
            )}
          </div>
          <button onClick={()=>setScreen("abitudini")} style={{fontSize:11,color:"rgba(255,255,255,0.25)",background:"transparent",border:"none",cursor:"pointer"}}>Tutte {"→"}</button>
        </div>
        ...
        {todayH.length===0?(
          ...
          <div>
            ...
            <div style={{borderRadius:14,overflow:"hidden",background:"rgba(255,255,255,0.02)",opacity:0.45}}>
              {allDoneList.map((h,i)=>(
                <div key={h.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.04)":"none",background:Z.s1}}>
                  <HRow h={h} tr={tr} logs={logs} onToggle={handleToggle} onEdit={onEditHabit} onLog={onLog} onDel={onDel} compact showArea/>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{borderRadius:14,overflow:"hidden",background:Z.s1}}>
            {rem.map((h,i)=>{
              ...
              return(
                <div key={h.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.04)":"none",
                  background:isFlash?"rgba(0,229,160,0.1)":Z.s1,
                  transition:"background 0.4s ease",position:"relative",overflow:"hidden"}}>
                  {isFlash&&(
                    <div style={{position:"absolute",left:16,top:"50%",width:32,height:32,marginTop:-16,borderRadius:"50%",background:"rgba(0,229,160,0.3)",pointerEvents:"none",animation:"rippleGreen 0.5s ease-out forwards"}}/>
                  )}
                  <HRow h={h} tr={tr} logs={logs} onToggle={handleToggle} onEdit={onEditHabit} onLog={onLog} onDel={onDel} compact showArea/>
                </div>
              );
        ...
        {allDoneList.length>0&&!allDone&&(
          <div style={{marginTop:10}}>
            ...
            {showDone&&(
              <div style={{borderRadius:12,overflow:"hidden",background:Z.s1,opacity:0.45,marginTop:6}}>
                {allDoneList.map((h,i)=>(
                  <div key={h.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.04)":"none",background:Z.s1}}>
                    <HRow h={h} tr={tr} logs={logs} onToggle={handleToggle} onEdit={onEditHabit} onLog={onLog} onDel={onDel} compact showArea/>
                  </div>
      ...
      {/* TASK */}
      {todayTasks.length>0&&(
        <div className="up d2" style={{padding:"4px 16px 0"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.3)",letterSpacing:"0.12em",textTransform:"uppercase"}}>Task in scadenza oggi</span>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={()=>setScreen("task")} style={{fontSize:11,color:"rgba(255,255,255,0.25)",background:"transparent",border:"none",cursor:"pointer"}}>{"Tutti →"}</button>
            </div>
          </div>
          <div style={{borderRadius:14,overflow:"hidden",background:Z.s1}}>
            {todayTasks.map((t,i)=>{
              ...
              return(
                <div key={t.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.05)":"none",background:isUrgent?"rgba(248,113,113,0.03)":Z.s1}}>
                  <div style={{display:"flex",alignItems:"flex-start",gap:12,padding:"11px 16px",cursor:"pointer"}}
                    ...
                    <button onClick={e=>{e.stopPropagation();setData(d=>({...d,tasks:d.tasks.map(x=>x.id===t.id?{...x,done:!x.done}:x)}));}}
                      style={{width:24,height:24,borderRadius:"50%",flexShrink:0,padding:0,cursor:"pointer",
                        background:t.done?"#00e5a0":"transparent",
                        border:`2px solid ${t.done?"#00e5a0":isUrgent?"rgba(248,113,113,0.4)":"rgba(255,255,255,0.15)"}`,
                        display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {t.done&&<span style={{color:"#0a0a0a",fontSize:11,fontWeight:900}}>{"✓"}</span>}
                    </button>
                    <span style={{flex:1,fontSize:14,color:t.done?Z.t3:Z.t,fontWeight:t.done?400:500,
                      textDecoration:t.done?"line-through":"none",
                      letterSpacing:"-0.1px",lineHeight:1.35,wordBreak:"break-word"}}>
                      {t.text}
                    </span>
                    {isUrgent&&!t.done&&(
                      <span style={{fontSize:9,fontWeight:600,color:"#f87171",border:"1px solid rgba(248,113,113,0.3)",padding:"2px 7px",borderRadius:20,flexShrink:0}}>scaduta</span>
                    )}
                    {area&&(
                      <span style={{fontSize:9,color:area.color,fontWeight:600,padding:"2px 6px",border:`1px solid ${area.color}50`,borderRadius:5,whiteSpace:"nowrap",flexShrink:0,marginTop:1}}>{badgeNome}</span>
                    )}
                  </div>
                </div>
              );
...
function GoalCircle({avg}){
  ...
  return(
    <span style={{display:"flex",alignItems:"center",gap:3}}>
      ...
      <span style={{fontSize:11,color:Z.t3}}>{avg}%</span>
    </span>
  );
}
...
function App(){
  const [data,setData]=useState(()=>{
    try{
      ...
      // Usa INIT come fonte di verità per le abitudini
      p.aree=(INIT.aree||[]).map(initArea=>{
        ...
        return{...initArea,cat:(initArea.cat||[]).map(initCat=>{
          ...
          const h=(initCat.h||[]).map(initH=>{
            const sv=(savedCat.h||[]).find(h=>h.id===initH.id)||{};
            return{
              ...initH,
              s:sv.s||initH.s,
              nome:sv.nome||initH.nome,
      ...
      // Merge goals: usa INIT come base, preserva progressi utente
      const savedGoals=p.goals||[];
      p.goals=INIT.goals.map(initG=>{
        const sv=savedGoals.find(g=>g.id===initG.id);
        if(!sv)return {...initG};
        // Preserva i traguardi dell'utente ma mantieni struttura INIT
        ...
        return{...initG,...sv,tasks,pct,s:sv.s||initG.s};
      });
      // Aggiungi goals creati dall'utente non in INIT
      const initIds=new Set(INIT.goals.map(g=>g.id));
      const extraGoals=savedGoals.filter(g=>!initIds.has(g.id));
      p.goals=[...p.goals,...extraGoals];
      return p;
    }catch{return INIT;}
  });
    ...
    <div style={{maxWidth:480,margin:"0 auto",minHeight:"100dvh",
            ...
            <div style={{padding:"0 20px 0"}}>
              ...
              <div style={{background:Z.s2,borderRadius:14,overflow:"hidden",
                ...
                <button onClick={()=>{
                  ...
                  a.href=url;
                  a.download="meridian_"+new Date().toISOString().split("T")[0]+".json";
        ...
        <div onClick={()=>{setAddingH(false);setAddArea(null);}}
          ...
          <div onClick={e=>e.stopPropagation()}
            style={{width:"100%",maxWidth:480,background:Z.s1,
              ...
              borderTop:"2px solid rgba(200,200,212,0.3)"}}>
            <div style={{width:36,height:4,background:Z.b2,borderRadius:2,
              margin:"0 auto 20px"}}/>
      ...
      <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",
        ...
        <div style={{display:"flex",alignItems:"center",
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0,flex:1}}>
            ...
            <button onClick={()=>setShowSettings(true)}
              style={{background:"transparent",border:"none",
                color:Z.t3,fontSize:15,opacity:0.5,padding:"2px 4px",
                lineHeight:1,cursor:"pointer",flexShrink:0}}>⚙️</button>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:11,color:Z.t3,fontFamily:"'JetBrains Mono',monospace",flexShrink:0}}>
            {tdone}/{todayH.length}
          </span>
          <span style={{color:Z.t3,fontSize:11}}>·</span>
          <span style={{fontSize:11,color:Z.t3,flexShrink:0}}>
      ...
      {/* CONTENT */}
      <div style={{flex:1,overflowY:"auto",padding:"86px 14px 80px",
        ...
        <div style={{animation:"fadeup 0.22s cubic-bezier(0.16,1,0.3,1) both"}}
          ...
          {screen==="home"     &&<Home data={data} tr={tr} logs={logs} showToast={showToast} onDel={delHabit} onToggle={toggle} onEditTask={setEditT} onDelTask={delTask}
            ...
            onLog={h=>h.logType==="sveglia"?setWakeH(h):null}
