...
function Abitudini({data,tr,logs,onToggle,onEdit,onLog,onAdd,onDel,onChangeStatus,addingH,setAddingH}){
  ...
  ).filter(h=>(!filterArea||h.aId===filterArea)&&!SPECIAL_LOGS.includes(h.logType));
    ...
    <div>
      ...
      {byCat.length>0&&byCat.map((grp,gi)=>(
        <div key={grp.catId} style={{background:Z.s1,
          border:`1px solid ${Z.b}`,borderRadius:14,
          overflow:"hidden",marginBottom:gi<byCat.length-1?6:0}}>
          <div onClick={()=>setOpenCats(o=>({...o,[grp.catId]:!isCatOpen(grp.catId)}))}
            ...
            <span style={{fontSize:11,fontWeight:700,
              color:grp.color,letterSpacing:"0.05em",
...
function Obiettivi({data,setData,showToast,editG,setEditG,newG,setNewG}){
  ...
  return(
    <div style={{paddingBottom:20}}>
      ...
      {/* Angolo dei Sogni */}
      <div style={{marginTop:16,background:Z.s1,border:`1px solid ${Z.b}`,
        borderRadius:12,overflow:"hidden"}}>
        <div style={{padding:"10px 14px 8px",borderBottom:`1px solid ${Z.b}`,
          display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:13}}>✨</span>
          <span style={{fontSize:12,fontWeight:700,color:"rgba(196,181,253,0.8)",
            letterSpacing:"0.05em",textTransform:"uppercase",
            fontFamily:"'JetBrains Mono',monospace",flex:1}}>
            Angolo dei Sogni
          </span>
          <span style={{fontSize:10,color:Z.t3}}>idee per il futuro</span>
        </div>
        {dreams.map((d,i)=>(
          <div key={d.id} style={{display:"flex",alignItems:"center",gap:8,
            padding:"9px 14px",borderTop:i===0?"none":`1px solid ${Z.b}`}}>
            <span style={{fontSize:11,color:Z.t3,flex:1,lineHeight:1.4}}>
              {d.text}
            </span>
            <button onClick={()=>delDream(d.id)}
              style={{color:Z.t3,fontSize:14,opacity:0.2,padding:"0 2px",
        ...
        <div style={{display:"flex",gap:6,padding:"8px 12px",
          ...
          <input value={newDream} onChange={e=>setNewDream(e.target.value)}
            ...
            placeholder="Scrivi un sogno..."
            style={{flex:1,background:Z.s2,
              border:"1px solid rgba(196,181,253,0.15)",
              borderRadius:8,padding:"8px 10px",
              color:Z.t,fontSize:13,outline:"none"}}/>
          <button onClick={saveDream}
            style={{padding:"8px 12px",background:"rgba(196,181,253,0.12)",
              border:"1px solid rgba(196,181,253,0.2)",
              borderRadius:8,color:"rgba(196,181,253,0.8)",
              fontSize:14,fontWeight:800,flexShrink:0}}>+</button>
        </div>
      </div>
    </div>
  );
    ...
    try{
      const resp=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        headers:{
          ...
          "x-api-key":localStorage.getItem("m-apikey")||"",
        ...
        body:JSON.stringify({
          ...
          max_tokens:1000,
          system:`Sei Lucius, life coach saggio e diretto di Davide. Rispondi sempre in italiano, in modo conciso e caldo ma non retorico. Sii diretto e pratico.
...
${context}`,
      ...
      const text=(d.content&&d.content[0]&&d.content[0].text)||"Nessuna risposta ricevuta.";
...
function SwipeTask({t,onToggle,onEdit,onDelete,aree,urgent}){
  ...
  const aCol=(aree&&aree.find(a=>a.id===t.aId)||{}).color;
  const aName=(aree&&aree.find(a=>a.id===t.aId)||{}).nome;
  useEffect(()=>{
        ...
        <div style={{flex:1,minWidth:0,cursor:"pointer",display:"flex",
          ...
          {aName&&(
            <span style={{fontSize:9,color:aCol,fontWeight:600,
              padding:"2px 7px",border:`1px solid ${aCol}50`,
...
function Home({data,tr,logs,onToggle,onEditHabit,onLog,setScreen,setData,onAddTask,showToast,onDel,onEditTask,onDelTask}){
  ...
  const HOME_ORDER=["h1","h14","h2","h6","h5","h10","h3","h51"];
    ...
    <div style={{paddingBottom:80}}>
                ...
                <div key={t.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.05)":"none",background:isUrgent?"rgba(248,113,113,0.03)":Z.s1}}>
                  <div style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",cursor:"pointer"}}
                    ...
                    <span style={{flex:1,fontSize:14,color:t.done?Z.t3:Z.t,fontWeight:t.done?400:500,
                      textDecoration:t.done?"line-through":"none",
                      overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"-0.1px"}}>
                      {t.text}
                    </span>
                    {isUrgent&&!t.done&&(
                    ...
                    {area&&(
                      <span style={{fontSize:9,color:area.color,fontWeight:600,padding:"2px 7px",border:`1px solid ${area.color}50`,borderRadius:20,whiteSpace:"nowrap",flexShrink:0}}>{area.nome}</span>
                    )}
                  </div>
                </div>
              );
      ...
      {todayTasks.length===0&&(
        <div style={{padding:"0 16px",display:"flex",justifyContent:"flex-end",gap:8}}>
          <button onClick={onAddTask} style={{width:28,height:28,borderRadius:"50%",background:"transparent",border:"1.5px solid rgba(0,229,160,0.5)",color:"#00e5a0",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>{"+"}</button>
          <button onClick={()=>setScreen("task")} style={{fontSize:11,color:"rgba(255,255,255,0.25)",background:"transparent",border:"none",cursor:"pointer"}}>{"Tutti →"}</button>
        </div>
      )}
...
function App(){
  const [data,setData]=useState(()=>{
    try{
      ...
      p.goals=(p.goals||[]).map(g=>({...g,pct:(g.tasks&&g.tasks.length)?Math.round(g.tasks.filter(t=>t.done).length/g.tasks.length*100):0,s:g.s||"red",note:g.note||""}));
      return p;
    }catch{return INIT;}
  });
              ...
              <div style={{background:Z.s2,borderRadius:14,overflow:"hidden",
                ...
                <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                  <div style={{width:36,height:36,borderRadius:10,
                    ...
                    fontSize:18,flexShrink:0}}>🧠</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:700,color:Z.t,marginBottom:2}}>API Key Anthropic</div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"86px 14px 80px",
        ...
        <div style={{animation:"fadeup 0.22s cubic-bezier(0.16,1,0.3,1) both"}}
          ...
          {screen==="abitudini"&&<Abitudini data={data} tr={tr} logs={logs} onDel={delHabit} onToggle={toggle}
            ...
            onEdit={h=>setEditH({...h})}
            onLog={h=>h.logType==="sveglia"?setWakeH(h):null} onAdd={addHabit} addingH={addingH} setAddingH={setAddingH}/>}
