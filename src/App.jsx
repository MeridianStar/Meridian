      ...
      {byCat.length>0&&byCat.map((grp,gi)=>(
        <div key={grp.catId} style={{background:Z.s1,
          ...
          {isCatOpen(grp.catId)&&grp.habits.map((h,i)=>(
            <div key={h.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.06)":"none"}}>
...
function GoalCard({g,openG,setOpenG,addTo,setAddTo,newT,setNewT,togT,delT,addT,updT,pctColor,setEditG}){
  ...
  const doReorder=(fromId,toId,tasksList)=>{
    if(!fromId||fromId===toId)return;
    const tasks=[...tasksList];
    const fi=tasks.findIndex(x=>x.id===fromId);
    const ti=tasks.findIndex(x=>x.id===toId);
    if(fi===-1||ti===-1)return;
    const [mv]=tasks.splice(fi,1);
    tasks.splice(ti,0,mv);
    return tasks;
  };

  const handleTouchStart=(t,tasksList)=>e=>{
    const touch=e.touches[0];
    touchStartY.current=touch.clientY;
    longPressTimer.current=setTimeout(()=>{
      setDraggingId(t.id);
      dragTask.current=t.id;
      if(navigator.vibrate)navigator.vibrate(10);
    },350);
  };
  const handleTouchMove=(tasksList)=>e=>{
    if(!draggingId){
      // Se si muove troppo prima del longpress, annulla il longpress (è uno scroll)
    ...
    const row=el&&el.closest("[data-task-id]");
    if(row){
      const overId=row.getAttribute("data-task-id");
      if(overId&&overId!==dragOver)setDragOver(overId);
    }
  };
  const handleTouchEnd=(tasksList)=>()=>{
    clearTimeout(longPressTimer.current);
    if(draggingId&&dragOver&&draggingId!==dragOver){
      const newTasks=doReorder(draggingId,dragOver,tasksList);
      if(newTasks)updT(g.id,newTasks,"reorder");
    }
    setDraggingId(null);setDragOver(null);dragTask.current=null;
  };
          ...
          {gt.map((t,i)=>{
            ...
            return(
              <div key={t.id}
                  data-task-id={t.id}
                  draggable
                  onDragStart={()=>{dragTask.current=t.id;}}
                  onDragOver={e=>{e.preventDefault();setDragOver(t.id);}}
                  onDrop={e=>{
                    e.preventDefault();
                    const newTasks=doReorder(dragTask.current,t.id,gt);
                    if(newTasks)updT(g.id,newTasks,"reorder");
                    dragTask.current=null;setDragOver(null);
                  }}
                  onDragEnd={()=>{dragTask.current=null;setDragOver(null);}}
                  onTouchStart={handleTouchStart(t,gt)}
                  onTouchMove={handleTouchMove(gt)}
                  onTouchEnd={handleTouchEnd(gt)}
                  style={{padding:"7px 11px",
                    background:dragOver===t.id?"rgba(255,255,255,0.06)":"transparent",
                    opacity:draggingId===t.id?0.4:1,
                    transition:"background 0.15s,opacity 0.15s",
                    touchAction:draggingId?"none":"auto",
                    borderTop:i===0?"none":`1px solid ${Z.b}`}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <span style={{fontSize:13,color:Z.t3,opacity:0.4,cursor:"grab",
                    flexShrink:0,letterSpacing:"-2px",userSelect:"none"}}>⠿</span>
                    ...
                    <React.Fragment>
                      ...
                      <button
                        onClick={e=>{e.stopPropagation();
                          ...
                          else{setEditingTId(t.id);setEditTxt(t.text);}
                          setEditSlider(prev=>prev===t.id?null:t.id);}}
                ...
                {editSlider===t.id&&(
                  <div onClick={e=>e.stopPropagation()} style={{paddingLeft:26,paddingBottom:4}}>
...
const HOME_ORDER=["h1","h2","h14","h10","h3","h51"];
          ...
          <div style={{flex:1,overflowY:"auto",padding:"12px 16px",
            ...
            {sorted.length===0?(
              ...
              sorted.map(n=>{
                ...
                const secondLine=(n.text||"").trim().split("\n").slice(1).join(" ").trim();
                return(
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0,flex:1}}>
            ...
            <button onClick={()=>setShowNotes(true)}
              style={{background:"rgba(255,255,255,0.06)",border:"none",
                opacity:0.85,padding:"6px",borderRadius:9,
