...
function HRow({h,tr,logs,onToggle,onEdit,onLog,onDel,onChangeStatus,compact,showArea,grid,flat}){
  ...
  if(grid){
    ...
    return(
      <div onClick={()=>onEdit(h)} style={{minHeight:0,minWidth:0,display:"flex",alignItems:"center",
        ...
        {isNone?(
          <button
            ...
            style={{width:15,height:15,borderRadius:"50%",flexShrink:0,padding:0,
              background:sc.c,border:"none",boxShadow:`0 0 5px ${sc.c}60`,cursor:"pointer"}}/>
        ):(
              ...
              <div key={t.id}
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  ...
                  <span onClick={e=>{e.stopPropagation();
                    ...
                    style={{fontSize:11,fontWeight:700,color:tc,
                    fontFamily:"'JetBrains Mono',monospace",flexShrink:0,
                    cursor:"pointer",padding:"2px 4px",borderRadius:5,
                    background:editSlider===t.id?`${tc}1a`:"transparent"}}>
                    {tpct}%
                  </span>
      ...
      <div className="up d1" style={{padding:"16px 16px 0",marginBottom:22}}>
        ...
        {todayH.length===0?(
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:5,opacity:0.45}}>
              {allDoneList.map(h=>(
                <HRow key={h.id} h={h} tr={tr} logs={logs} onToggle={handleToggle} onEdit={onEditHabit} onLog={onLog} onDel={onDel} compact grid flat/>
              ))}
            </div>
          </div>
        ):(
            ...
            {rem.map(h=>{
              ...
              return(
                <div key={h.id} style={{position:"relative",overflow:"hidden",borderRadius:10,
                  ...
                  <HRow h={h} tr={tr} logs={logs} onToggle={handleToggle} onEdit={onEditHabit} onLog={onLog} onDel={onDel} compact grid flat/>
                </div>
              );
            })}
          </div>
        ...
        {allDoneList.length>0&&!allDone&&(
          <div style={{marginTop:8}}>
            ...
            {showDone&&(
              <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:5,opacity:0.45,marginTop:6}}>
                {allDoneList.map(h=>(
                  <HRow key={h.id} h={h} tr={tr} logs={logs} onToggle={handleToggle} onEdit={onEditHabit} onLog={onLog} onDel={onDel} compact grid flat/>
                ))}
              </div>
